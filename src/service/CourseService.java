...
public class CourseService {
    ...
    private final Map<String, Course> courses = new HashMap<>();                    // stores courses in the map
    private final Map<String, List<Student>> enrolledStudents = new HashMap<>();    // stores list of students in the map

    public CourseService() {                                                        // empty constructor where we will populate our courses
    ...
    // void cos not returning anything
    ...
    public void enrollStudent(String courseId, Student student){                                        // enroll a student to a course
        // How should we save the course and the student enrolled into a course
        // In order to enroll a student, need to check whether the course exist in the first place
        if(courses.containsKey(courseId)){                                                              // if courseId is not found in enrolledStudent
    ...
    //show everything that is why there are no perimeter's set
    public void showSummary(){                                                      // show summary of courses and enrolled students
        ...
        for (String key: enrolledStudents.keySet()){                                // iterate through the courses found in enrolledStudents
            ...
            // Note: each course's enrolledStudents is stored as an arrayList
            List<Student> students = enrolledStudents.get(key);                     // store the students from each key (courseId)

            System.out.println("Students in Course Code:" + key);                   // Print the key, i.e. the courseId
